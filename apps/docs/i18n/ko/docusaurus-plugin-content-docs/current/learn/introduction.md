...
| **테마 시스템** | Tailwind CSS와 CSS 변수 커스터마이징을 사용한 직접 임포트 테마. 라이트/다크/시스템 모드 기본 제공 |
...
| **배포 옵션** | Vercel (원클릭) 또는 Docker 셀프호스팅 |
...
테마는 React 컴포넌트, 레이아웃, 템플릿을 직접 내보내는 npm 패키지입니다. 필요한 것을 임포트하세요 — 프로바이더나 컨트랙트 불필요. 기본 테마는 블로그와 문서 페이지 타입을 지원하며, 모든 컴포넌트는 Tailwind CSS를 사용합니다. [테마](./themes)를 참조하세요.
