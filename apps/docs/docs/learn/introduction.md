...
| **Theme system** | CSS variable theming with `extendTheme()` for inheritance. Light/dark/system modes out of the box |
...
Themes are CSS variable packages that can be extended via `extendTheme()`. The default theme supports blog, docs, and portfolio page types. Community themes can declare which page types they support. See [Themes](./themes).
